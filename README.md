...
首先，我们有必要了解下什么是动画。动画，相信大家都不陌生。
简单来说就是在一定的时间快速的连续播放序列图像（关键帧），就像电影一样
在我看来，动画就是在一个空间一定时间内每个对象按照固定的运动轨迹呈现的画面。
### FPS （每秒传输帧数(Frames Per Second)）
> FPS是图像领域中的定义，是指画面每秒传输帧数

   相信玩过动画的人都知道fps，通俗来讲就是每秒的关键帧数。那么多少的fps是最佳的呢？因为每秒钟帧数愈多，所显示的动作就会愈流畅。理论上来说，肯定是多多益善的。
然而，实际并非如此，其中原因是在显示“分辨率”不变的情况下，FPS越高，则对显卡的处理能力要求越高。电脑的显示画面是通过显卡进行输出的，假设分辨率是1024×768时，
画面的刷新率要达到24帧/秒，那么显卡在一秒钟内需要处理的像素量就达到了“1024×768×24=18874368”，可想而知，这种频率的增加的数量级是很恐怖的。一般来说，对于显示器默认的刷新频率在85hz左右。
...
这里可以看到10-12帧只是连贯而已。15.75kHz是人体对显示器最低要求的刷新频率，一般人不觉得卡顿的FPS频率大约是30Hz（fps），想要达到流畅等级则需要60Hz（fps），也就是16.66ms
一帧。其实，我们前端，主要的载体还是浏览器。然而浏览器又有一套自己的repaint的频率。这里就必须提到强大的requestAnimationFrame(后面会详细介绍)
...
我们知道几乎canvas API所有都是在context上调用的。然而我们可能并不是很清楚context是一个状态机。
...
这个相信大家都很熟悉了，做动画都最佳循环方法。什么意思呢？就是前面提到都最佳都人眼感受是16.67ms一帧。
